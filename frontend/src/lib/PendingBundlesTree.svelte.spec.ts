...
describe('PendingBundlesTree.svelte', () => {
	...
	it('lets admins stage and commit bundles that are ready for pickup', async () => {
		...
		await expect.element(page.getByText('ready status pending update')).toBeInTheDocument();
		...
		await vi.waitFor(() => {
			...
			expect(body.bundled).toBe(true);
			expect(body.fulfilled_at).toBeNull();
			expect(body.items).toEqual([{ item_id: 10, quantity: 5 }]);
		});
	});

	it('lets members confirm pickup of read-only ready bundles', async () => {
		sessionStorage.setItem('hearthledger.user.id', 'member-a');

		const fetchMock = vi.spyOn(window, 'fetch').mockImplementation(async (input, init) => {
			const url = String(input);

			if (url === '/api/users') {
				return Response.json([{ id: 'member-a', name: 'Member A', role: 'member' }]);
			}
		...
		await expect.element(page.getByText('pickup pending update')).toBeInTheDocument();
		await page.getByRole('button', { name: 'update' }).click();
